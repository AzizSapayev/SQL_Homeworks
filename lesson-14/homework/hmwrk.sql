...
ParseName(Replace(Name,',','.'),1) as SecondName
...
Select 
ParseName(Replace(Vals,'.','.'),3) as FirstName,
ParseName(Replace(Vals,'.','.'),2) as SecondName,
ParseName(Replace(Vals,'.','.'),1) as SecondName
from Splitter

SELECT TRANSLATE('1234ABC123456XYZ1234567890ADS', 
                 '0123456789', 
                 'XXXXXXXXXX') AS ReplacedString;
Select * from Employees
where CHARINDEX('.',Vals)>=2
Select COUNT(' ') from CountSpaces

Select Name ,Salary, ManagerId from Employee
where Salary>(Select MAx(Salary) from Employee where ManagerId is not null)
...
and DATEDIFF(YEAR,HIRE_DATE,GETDATE())<55

SELECT * from weather
SELECT w1.Id, w1.RecordDate, w1.Temperature
FROM Weather w1
JOIN Weather w2 
  ON w1.RecordDate = DATEADD(DAY, 1, w2.RecordDate)   -- SQL Server
  -- ON w1.RecordDate = DATE_ADD(w2.RecordDate, INTERVAL 1 DAY)  -- MySQL
WHERE w1.Temperature > w2.Temperature;

Select * from fruits
SELECT STUFF('Hello World', 7, 5, 'SQL');
SELECT SYSDATETIME() AS Aniqlik;
...
SELECT 
  ...
  SELECT 
  DATEDIFF(DAY, '2022-01-01', GETDATE()) AS KunFarqi,
  DATEDIFF(YEAR, '1444-05-01', GETDATE()) AS YilFarqi;

  SELECT EOMONTH(GETDATE()) AS OyOxiri;
  Select GETUTCDATE()
  Select * from p1
  Select * from p2
...
SELECT 
    ...
    p1.code,
    CASE 
        WHEN p1.id = 0 THEN REPLACE(CAST(p2.id AS VARCHAR(50)), '0', 'X')
        ELSE REPLACE(CAST(p2.id AS VARCHAR(50)), '1', 'Y')
    END AS Changing
FROM p1
