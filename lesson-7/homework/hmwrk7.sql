...
where DepartmentName in ('HR','IT','Finance','Marketing')
...
SELECT 
    FirstName  as CustomerName, COUNT(Orders.Quantity) as Orders
FROM 
    Customers
JOIN 
...
GROUP BY 
   Customers.FirstName
   Having COUNT(Orders.Quantity)>=3
...
HAVING SUM(Sales.SaleAmount) > 1500;
