...
Select CAST(EMPLOYEE_ID as varchar)+'-'+FIRST_NAME+' '+LAST_NAME from Employees
Update Employees
Set PHONE_NUMBER=REPLACE(phone_number,'124','999')
Where PHONE_NUMBER like '%124%'

Select FIRST_NAME+' '+LAST_NAME,len(FIRST_NAME+' '+LAST_NAME) from Employees
where (FIRST_NAME+' '+LAST_NAME)Like '[AJM]%'

Select Sum(SALARY),MANAGER_ID from Employees
Group by MANAGER_ID

Select MAX(Max1),MAX(Max2),MAX(Max3) from TestMax
...
Select * from Student
Select Cast(SequenceNumber as varchar(20))+' '+String from DMLTable

Select FIRST_NAME+' '+LAST_NAME as FullName from Employees
WHERE LEN(FIRST_NAME+LAST_NAME)-LEN(Replace(FIRST_NAME+LAST_NAME,'e',''))>=3
Select * from Employees
SELECT 
...
sELect MAX(SpacemanID),MIN(SpacemanID) from Personal

Declare @myy varchar(40)='tf56sd#%OqH';
-- Uppercase letters
SELECT 
   STRING_AGG(value, '') AS Uppercase
FROM STRING_SPLIT('tf56sd#%OqH','')
WHERE value COLLATE Latin1_General_CS_AS LIKE '[A-Z]';
...
FROM Students;
-- Add a column for storing the result
ALTER TABLE Equations
ALTER COLUMN TotalSum FLOAT;

-- Cursor to loop through each equation
DECLARE @Equation NVARCHAR(100), 
        @SQL NVARCHAR(200), 
        @Result FLOAT;
...
BEGIN
    -- Build dynamic SQL
    SET @SQL = N'SELECT @ResOUT = ' + @Equation;

    -- Execute the equation and get the result
    EXEC sp_executesql @SQL, N'@ResOUT FLOAT OUTPUT', @ResOUT=@Result OUTPUT;
